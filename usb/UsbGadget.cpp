...
UsbGadget::UsbGadget() {
    // Upstream (hardware/interfaces/usb/gadget/1.1/default) aborts here immediately if
    // OS_DESC_PATH isn't readable yet. That's what caused a prior attempt on this device to
    // regress into an infinite "Starting phone..."/bootanim loop on real hardware: if the
    // vendor rc's "on post-fs" symlink creation (init.mt6768.usb.rc) is ever slower than this
    // constructor running, the whole process crash-loops forever and UsbDeviceManager's
    // blocking IUsbGadget.getService(true) never unblocks either - the exact same symptom this
    // HAL exists to fix in the first place. Wait instead of dying immediately - by the time
    // class_start hal runs this should already exist per normal init trigger ordering, so this
    // is a bounded safety margin, not the expected path.
    for (int i = 0; i < 50 && access(OS_DESC_PATH, R_OK) != 0; i++) {
        usleep(100000);
    }
    if (access(OS_DESC_PATH, R_OK) != 0) {
        ALOGE("configfs setup still not done after waiting - proceeding anyway, function"
              " composition may fail until it appears");
    }
}
